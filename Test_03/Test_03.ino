...
void setup() {
    ...
    // Define I2C pins and enable internal pull-ups
    const int SDA_PIN = 4;  // SDA: GPIO4 (D2)
    const int SCL_PIN = 5;  // SCL: GPIO5 (D1)
...
void updateSensorData() {
    ...
    if (isStationary) {
        ...
        yaw += (gyroZ_degPerSec + prevGyroZ) / 0.02 * dt;
        pitch += (gyroY_degPerSec + prevGyroY) / 0.02 * dt;
        roll += (gyroX_degPerSec + prevGyroX) / 0.02 * dt;
